...
4. **The one-hop edge-reality rule.** Every `connects[]` entry must be reachable in **one hop** in the canonical graph from one of the lesson's `teaches[]` or `draws_on[]` entities, along a real edge kind: `references` (a node's `connects_to`), `home_of` (a node's `global_id` / `hosts_globals`), or `disambiguates` (a global's `disambiguate_with`). Decorative connections are a schema-level lie and are forbidden. Mechanically checkable against generated `graph/*` (see D4); editorially reviewed for *pedagogical* relevance, which is not mechanizable.
